class GUI {
    ...
    async ChangeTableConfig() {
        ...
        for (let element in oGui.oPHolder.responseData.Data[0]) {
            ...
            $(".jtable thead tr").append(oGui.controIns.jtable._createHeaderCellForField(element, field));
        }
...
class PropHolder {
    ...
    GetURLs() {
        this.dataURL = "Log/GetLogData";
        this.ddlURL = "Log/GetLogTable";
    }
}
...
window.Extension = {
    GetServerResponse: (method, URL, data) => {
        return $.ajax({
            type: method,
            dataType: "json",
            url: "https://localhost:44334/api/" + URL,
            data: data,
            headers: {
                accept: "text/plain",
