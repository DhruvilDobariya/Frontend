...
function getAllStudent(){
    $.ajax({
        ...
        $(JSON.parse(json)).each(function(i,value){
            $("tbody").append('<tr><th scope="row" class="no">'+(i+1)+'</th><td>'+value.Id+'</td><td>'+value.Name+'</td><td>'+value.RollNo+'</td><td>'+value.Email+'</td><td>'+value.ContactNo+'</td><td class="d-flex justify-content-center"><button class="btn btn-success mx-2 edit"><i class="fas fa-user-edit"></i></button><button class="btn btn-danger mx-2 delete"><i class="fas fa-trash-alt"></i></i></button></td></tr>');
        })
    })
}
...
function getStudentById(id){
    ...
    $.ajax({
        type : "GET",
        url : "https://localhost:44319/api/students/" + id,
        data : {},
        datatype : "json",
        header : {
        }
